...
/***********************************************************************/
{
   ...
   else
   {
      ...
      for ( i = 0; i < VIEW_WINDOWS; i++ )
      {
         ...
         if ( row >= top_int_row
         &&   row <= bot_int_row
         &&   col >= (screen[curr_screen].start_col[i]  - screen[curr_screen].screen_start_col)
         &&   col <= (screen[curr_screen].start_col[i] + screen[curr_screen].cols[i] - 1 - screen[curr_screen].screen_start_col) )
         {
            idx = i;
            break;
         }
