...
/***********************************************************************/
{
   ...
   LENGTHTYPE start_col=0,real_start=0,real_end=0,loc=0;
   ...
   LENGTHTYPE len_old_str=0,len_new_str=0;
   TARGET target;
...
#ifdef WIN32
   /*
    * We need to call resume_curses() to reinstate ^C and ^S handling
    * after running a command with OSQ
    */
   else if (quiet && curses_started)
      resume_curses();
#endif
   if (curses_started)
      draw_cursor(TRUE);
...
/***********************************************************************/
{
   ...
   LENGTHTYPE left_col=0L,right_col=0L;
   LENGTHTYPE j=0;
   LENGTHTYPE actual_cols=0;
   short rc=RC_OK;
...
/***********************************************************************/
{
   ...
   {
      /*
       * We found a line with the specified name; remove it from
       * the list of names for this line
...
/***********************************************************************/
{
   ...
   else
   {
      ...
      {
         /*
          * Remove leading and trailing spaces, then remove any leading and trailing
          * double quotes
          */
         target.rt[target.spare].string = MyStrip( MyStrip( target.rt[target.spare].string, STRIP_BOTH, ' ' ), STRIP_BOTH, '"' );
...
/***********************************************************************/
{
   ...
   if ( rc == RC_TARGET_NOT_FOUND )
   {
      ...
      {
         /*
          * Put the current and focus lines back the way they were
          * before wrapping.
          */
