/*
 * Altered for use in THE, by Mark Hessling; entry point called my_getopt()
 * to not conflict with getopt() in standard libraries.
 */
/* Getopt for GNU.
...
#endif
{
  ...
  if (longopts != NULL
    ...
    {
      const struct my_getopt_option *p;
      char *s = nextchar;
      int exact = 0;
      int ambig = 0;
      const struct my_getopt_option *pfound = NULL;
      int indfound=0;
...
#endif
{
  return _getopt_internal (argc, argv, optstring,
                     (const struct my_getopt_option *) 0,
                     (int *) 0,
                     0);
}
