...
#if WANT_TO_PASS_REGS_BACK
/* this has been commented out because on WIndows XP 64bit, regs gets a value
 * and causes THE to crash */
          if (regs && !bufp->no_sub)
       {
   ...
   case charset_not:
     {
       ...
       boolean nott = (re_opcode_t) *(p - 1) == charset_not;
   ...
   case on_failure_jump:
      ...
      p1 = p + mcnt;
        ...
        {
          int nott = (re_opcode_t) p1[3] == charset_not;
...
/*
 * MH 12/5/2001
 * Removed use of global variable; re_syntax_options. Instead it is passed to
 * re_compile_pattern(). This may be non-portable, but is better practice.
 */
#ifdef __cplusplus
...
#else
const char *
re_compile_pattern (pattern, re_syntax_options, length, bufp)
     const char *pattern;
     reg_syntax_t re_syntax_options;
     int length;
     struct re_pattern_buffer *bufp;
 ...
 * MH 12/5/2001
 * Removed use of global variable; re_syntax_options. Instead it is passed to
 * the_re_comp(). This may be non-portable, but is better practice.
 * Need to rename it as it conflicts with BSD re_comp()
 */
#ifdef __cplusplus
char *the_re_comp(
    const char *s,
...
#else
char *
the_re_comp (s,re_syntax_options)
    const char *s;
    reg_syntax_t re_syntax_options;
#endif
{
...
#else
int
the_regcomp (preg, pattern, cflags)
    regex_t *preg;
    const char *pattern;
    int cflags;
#endif
...
#else
int
the_regexec (preg, string, nmatch, pmatch, eflags)
    const regex_t *preg;
    const char *string;
    size_t nmatch;
    regmatch_t pmatch[];
...
#else
size_t
the_regerror (reg_errcode, preg, errbuf, errbuf_size)
    int reg_errcode;
    const regex_t *preg;
    char *errbuf;
    size_t errbuf_size;
#endif
{
  ...
  if (reg_errcode < 0
      || reg_errcode >= (sizeof (re_error_msg) / sizeof (re_error_msg[0])))
